...
module.exports = function(db){
    return {
        findById:function (id) {
            var d = new Deferred();
            db.collection('applications').findOne({_id:db.ObjectID(id)}, function (error, result) {
                if (error)
                    d.fail(error);
                else
                    d.call(result)
        ...
        findAll:function (callback) {
            var d = new Deferred();
            db.collection('applications').find().toArray(function (error, results) {
                if (error)
                    d.fail(error);
                else
                    d.call(results)
        ...
        findAllByUserId:function (id, callback) {
            ...
            db.collection('applications').find({'author.$id':id}).toArray(function (error, results) {
                if (error) callback(error);
                else callback(null, results)
            });
        },
        findAllLinks:function (callback) {
            db.collection('applications').find({}, {'_id':1, 'name':1}).toArray(function (error, results) {
                if (error) callback(error);
                else callback(null, results)
            });
        },
        insert:function (data, callback) {
            ...
            //data.parent = new db.db.bson_serializer.DBRef('applications',db.ObjectID(data.parent+""),null);
            db.collection('applications').insert(data, function (error, result) {
                    if (error) callback(error);
        ...
        update:function (data, callback) {
            if (data.parent != undefined){
                data.parent = new db.db.bson_serializer.DBRef('applications',data.parent);
            }
